...
const CommunityFeed: React.FC<Props> = ({ user, theme }) => {
  ...
  useEffect(() => {
    if (isCreatingPost || isCreatingStory || activeStoryIndex !== null) {
      document.body.style.overflow = 'hidden';
    } else {
      document.body.style.overflow = '';
    }
    return () => { document.body.style.overflow = ''; };
  }, [isCreatingPost, isCreatingStory, activeStoryIndex]);
  ...
  return (
    <div className="relative min-h-screen bg-brand-silk dark:bg-brand-obsidian transition-colors overflow-x-hidden">
      ...
      {isCreatingStory && (
        <div className="fixed inset-0 z-[10000] bg-black flex flex-col animate-in slide-in-from-bottom duration-300 pt-safe">
          {/* Header Flotante */}
        ...
        <div className="fixed inset-0 z-[10000] bg-black flex items-center justify-center animate-in fade-in duration-200">
          ...
          <div className="relative w-full h-full md:max-w-md md:h-[90vh] md:rounded-3xl bg-black overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="absolute top-8 right-4 z-[60] flex gap-4 items-center">
              ...
              {localStories[activeStoryIndex].userId === user.id && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (confirm('¿Eliminar esta historia?')) {
                      deleteStoryMutation.mutate(localStories[activeStoryIndex].id);
                    }
                  }}
